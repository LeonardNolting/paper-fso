...
Given those parameters, 12.2\% of candidate links are unobstructed and therefore theoretically suitable for an FSO link.
... short links more interesting, quality, uptime ...
Contradiction:
Short links more likely to be clear (results confirm that)
But the further out you go, the more candidates there are
What is it really like? → Figure
...
More relevant than the absolute amount of links is the number of links per node.
- 200m = majority
- 300m = 3/4
...
operators: Can a single operator cover a city? What link length is required? → connected components

single operator not very good

operator cooperations better

full network even very good

connected components may not be very stable → k-connectivity, Menger's theorem, disjoint paths (again for operator cooperations and full network?)

redundancy per node, degrees (is that implied by k-connectivity?), link failure resilience

spatial correlation of link failures → angular diversity of links (probability of a new obstacle isolating a node, depending on obstacle size)
